...
from tip_calibration import TipCalibration  # Import the TipCalibration class
...
def perform_scan_and_process_images(nanonis, npy_output_folder, resized_output_folder, augmented_output_folder, filtered_save_dir, model_path, grid_manager, tip_calibration):
    try:
        ...
        while True:
            ...
            if not prediction_successful:
                ...
                # Perform controlled tip dip using TipCalibration class
                tip_calibration.perform_controlled_dip(initial_z_position_1V, dip_amount=1e-9)

                # Perform controlled retraction before moving to the next scan region
                tip_calibration.perform_controlled_retraction(initial_z_position_1V, retract_distance=100e-9, retract_steps=10, original_bias=1.0)
...
if __name__ == "__main__":
    ...
    # Initialize the TipCalibration class
    tip_calibration = TipCalibration(nanonis)

    # Start the scanning and image processing
    perform_scan_and_process_images(nanonis, npy_output_folder, resized_output_folder, augmented_output_folder, filtered_save_dir, model_path, grid_manager, tip_calibration)
